...
class TestLoadClusterData:
    ...
    @patch("stereomapper.comparison.compare.assemblers.build_mols_for_reps")
    @patch("stereomapper.comparison.compare.results_repo.fetch_cluster_reps_for_inchikey")
    def test_insufficient_clusters_after_build(
        self, mock_fetch_reps, mock_build_mols, mock_preload_pairs
    ):
...
class TestAnalyzePair:
    ...
    def test_successful_primary_analysis(self):
        ...
        with patch("stereomapper.comparison.compare.summary_logger"):
            with patch(
                "stereomapper.comparison.compare._is_valid_primary_result", return_value=True
            ):
                result = _analyze_pair(
                    self.mock_analyser,
                    self.mock_fallback_analyser,
    ...
    def test_primary_fails_fallback_succeeds(self):
        ...
        self.mock_fallback_analyser.analyze_relationship_fallback.return_value = (
            mock_fallback_result
        )

        with patch("stereomapper.comparison.compare.summary_logger"):
            with patch(
                "stereomapper.comparison.compare._is_valid_primary_result", return_value=False
            ):
                result = _analyze_pair(
                    self.mock_analyser,
                    self.mock_fallback_analyser,
...
class TestProcessResult:
    ...
    def test_none_result(self):
        ...
        with patch("stereomapper.comparison.compare.logger"):
            result = _process_result(
                None, "cluster1", "cluster2", "v1.0", '["mol1"]', '["mol2"]', 1, 1
            )
    ...
    def test_missing_classification(self):
        ...
        with patch(
            "stereomapper.comparison.compare.assemblers._normalise_classification",
            return_value=None,
        ):
            with patch("stereomapper.comparison.compare.logger"):
                result = _process_result(
                    mock_result, "cluster1", "cluster2", "v1.0", '["mol1"]', '["mol2"]', 1, 1
                )
    ...
    def test_successful_processing(self):
        ...
        with patch(
            ...
            with patch(
                "stereomapper.comparison.compare.assemblers._coerce_scalar", return_value=0.9
            ):
                with patch(
                    "stereomapper.comparison.compare.assemblers._details_from_res",
...
class TestCompareClusterRelationships:
    ...
    def test_no_data_loaded(self, mock_load_data):
        ...
        compare_cluster_relationships(
            results_db_path="test.db", inchikey_first="ABCD", version_tag="v1.0", logger=mock_logger
        )
    ...
    def test_successful_comparison(
        ...
        mock_process_result.return_value = (
            "cluster1",
            "cluster2",
            '["mol1"]',
            '["mol2"]',
...
def mock_analyzers():
    ...
    return {"primary": Mock(), "fallback": Mock()}
