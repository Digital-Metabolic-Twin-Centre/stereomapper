...
from stereomapper.domain.exceptions.processing import CacheError, ProcessingError
from stereomapper.domain.exceptions.validation import ValidationError
from stereomapper.runners.pipeline import run_mol_distance_2d_batch  # Add this line
from stereomapper.runners.pipeline import PipelineConfig, PipelineResult, StereomapperPipeline
...
class TestPipelineConfig:
    ...
    def test_pipeline_config_creation(self):
        ...
        config = PipelineConfig(input=["test.mol"], sqlite_output_path="output.db")
...
class TestPipelineResult:
    ...
    def test_pipeline_result_creation(self):
        ...
        result = PipelineResult(
            n_inputs=100, n_session_pairs=50, n_cross_pairs=25, processing_time=120.5
        )
...
class TestStereomapperPipeline:
    ...
    def test_validate_config_no_input(self, mock_psutil, temp_dir):
        ...
        config = PipelineConfig(
            input=None, input_dir=None, sqlite_output_path=str(temp_dir / "output.db")
        )
    ...
    def test_validate_config_no_output_path(self, mock_psutil):
        ...
        config = PipelineConfig(input=["test.mol"], sqlite_output_path=None)
    ...
    def test_initialize_and_validate(
        self, mock_validator, mock_resolve, mock_psutil, basic_config, mock_molfiles
    ):
    ...
    @patch("stereomapper.runners.pipeline.psutil")
    @patch("stereomapper.utils.itertools.chunked")
    ...
    @patch("stereomapper.runners.pipeline.psutil")
    @patch("stereomapper.utils.itertools.chunked")
    def test_process_molecules_success(
        self, mock_chunked, mock_psutil, basic_config, mock_molfiles
    ):
    ...
    def test_update_relationship_metrics(self, mock_connect, mock_psutil, basic_config, temp_dir):
        ...
        cur2.__iter__.return_value = iter(
            [('{"pair_origin": "session"}',), ('{"pair_origin": "cross"}',), (None,)]
        )
...
class TestPipelineIntegration:
    ...
    def test_run_pipeline_success(
        ...
        bp = mock_batch_processor.return_value
        ...
        # run
        from stereomapper.runners.pipeline import PipelineResult
    ...
    def test_run_pipeline_validation_error(
        self, mock_resolve_inputs, mock_psutil, integration_config
    ):
...
class TestLegacyFunction:
    ...
    def test_legacy_function_calls_pipeline(self, temp_dir):
        ...
        config = PipelineConfig(input=["test.mol"], sqlite_output_path=str(temp_dir / "output.db"))
...
class TestErrorHandling:
    ...
    def test_processing_error_propagation(self, mock_psutil, temp_dir):
        ...
        config = PipelineConfig(
            input=["nonexistent.mol"], sqlite_output_path=str(temp_dir / "output.db")
        )
    ...
    def test_cache_error_propagation(self, temp_dir):
        config = PipelineConfig(input=["test.mol"], sqlite_output_path=str(temp_dir / "output.db"))
        pipeline = StereomapperPipeline(config)
        ...
        with patch.object(pipeline, "_initialize_and_validate", return_value=["test.mol"]):
            ...
            with patch.object(
                pipeline, "_setup_cache_database", side_effect=CacheError("Cache failed")
            ):
                with patch.dict(os.environ, {"NO_PROGRESS": "1"}):
                    with pytest.raises(CacheError) as exc_info:
                        pipeline.run()
