...
class TestRelationshipAnalyser:
    ...
    def basic_params(self):
        """Basic parameters for calc_relationship tests."""
        return {
            "is_radio1": False,
            "is_radio2": False,
            "has_sru1": False,
    ...
    def test_invalid_mol_objects(self, analyser, basic_params):
        ...
        with pytest.raises(ValueError, match="Both inputs must be RDKit Mol objects"):
            analyser.calc_relationship("not_a_mol", "also_not_a_mol", **basic_params)
    ...
    def test_radioactive_mismatch(self, analyser, mock_mol1, mock_mol2, basic_params):
        ...
        basic_params["is_radio1"] = True
        basic_params["is_radio2"] = False
    ...
    def test_rmsd_error(
        self, mock_normalise, mock_align, analyser, mock_mol1, mock_mol2, basic_params
    ):
    ...
    @patch("stereomapper.classification.engine.ChemistryOperations.align_molecules")
    @patch("stereomapper.classification.engine.ChemistryUtils.normalise_rmsd")
    def test_stereo_elements_none(
        self, mock_normalise, mock_align, mock_stereo, analyser, mock_mol1, mock_mol2, basic_params
    ):
    ...
    def test_no_stereogenic_elements_identical(
        self,
        mock_builder_class,
        mock_inchi_class,
        mock_tanimoto,
    ...
    @patch("stereomapper.classification.engine.InChIFallbackAnalyser")
    @patch("stereomapper.classification.engine.FeatureBuilder")
    def test_no_stereogenic_elements_protomers(
        self,
        mock_builder_class,
        mock_inchi_class,
        mock_tanimoto,
    ...
    def test_diastereomers(
        self,
        mock_is_diastereomer,
        mock_is_enantiomer,
        mock_builder_class,
        mock_inchi_class,
        mock_tanimoto,
    ...
    def test_parent_child_structures(
        self,
        mock_classifier,
        mock_builder_class,
        mock_inchi_class,
    ...
    @patch("stereomapper.classification.engine.ChemistryOperations.fingerprint_tanimoto")
    @patch("stereomapper.classification.engine.InChIFallbackAnalyser")
    def test_inchi_layer_extraction_failure(
        self,
        mock_inchi_class,
        mock_tanimoto,
        mock_normalise,
    ...
    def test_missing_charge_identical(self, analyser, mock_mol1, mock_mol2, basic_params):
        ...
        with (
            patch(
                "stereomapper.classification.engine.StereoAnalyser.compare_stereo_elements"
            ) as mock_stereo,
            patch(
                "stereomapper.classification.engine.ChemistryOperations.align_molecules"
            ) as mock_align,
            patch(
                "stereomapper.classification.engine.ChemistryUtils.normalise_rmsd"
            ) as mock_normalise,
            patch(
                "stereomapper.classification.engine.ChemistryOperations.fingerprint_tanimoto"
            ) as mock_tanimoto,
            patch("stereomapper.classification.engine.InChIFallbackAnalyser"),
            patch("stereomapper.classification.engine.FeatureBuilder") as mock_builder_class,
        ):
