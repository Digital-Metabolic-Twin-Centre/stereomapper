...
class TestSettings:
    ...
    def test_validate_input_directory_not_directory(self, tmp_path):
        ...
        not_dir.touch()
        settings = Settings(
            input_directory=not_dir, database=DatabaseSettings(output_path=tmp_path / "output.db")
        )
        with pytest.raises(ConfigurationError) as exc_info:
            settings.validate()
    ...
    def test_validate_output_compatibility_missing_db_path(self, tmp_path):
        ...
        settings = Settings(input_files=[Path("file1.mol")])
        with pytest.raises(ConfigurationError) as exc_info:
            settings.validate()
    ...
    def test_validate_success_with_input_directory(self, tmp_path):
        ...
        settings = Settings(
            input_directory=tmp_path, database=DatabaseSettings(output_path=tmp_path / "output.db")
        )
        settings.validate()  # Should not raise
    ...
    def test_to_dict(self, tmp_path):
        ...
        settings = Settings(
            ...
            database=DatabaseSettings(
                cache_path=cache_path, output_path=output_path, fresh_cache=True
            ),
            namespace="test",
            debug_mode=True,
            dry_run=True,
