...
from stereomapper.config.resolvers import _resolve_cache_path
...
class TestResolveCachePath:
    def test_both_flags_true_raises_error(self):
        ...
        with pytest.raises(
            ValueError, match="Cannot set both relate_with_cache=True and fresh_cache=True"
        ):
    ...
    def test_relate_with_cache_missing_default_path(self, tmp_path):
        ...
        with patch("stereomapper.config.resolvers.default_cache_path", return_value=cache_file):
            with pytest.raises(
                FileNotFoundError,
                match=f"relate_with_cache=True but cache DB not found: {cache_file}",
            ):
    ...
    def test_relate_with_cache_existing_custom_path(self, tmp_path):
        ...
        result = _resolve_cache_path(
            relate_with_cache=True, fresh_cache=False, cache_path=str(cache_file)
        )
        assert result == cache_file
    ...
    def test_relate_with_cache_missing_custom_path(self, tmp_path):
        ...
        with pytest.raises(
            FileNotFoundError, match=f"relate_with_cache=True but cache DB not found: {cache_file}"
        ):
            _resolve_cache_path(
                relate_with_cache=True, fresh_cache=False, cache_path=str(cache_file)
            )
    ...
    def test_fresh_cache_with_existing_custom_file(self, tmp_path):
        ...
        result = _resolve_cache_path(
            relate_with_cache=False, fresh_cache=True, cache_path=str(cache_file)
        )
        assert result == cache_file
        assert not cache_file.exists()  # Should be deleted
    ...
    def test_neither_flag_with_default_path(self, tmp_path):
        ...
        with patch("stereomapper.config.resolvers.default_cache_path", return_value=cache_file):
            result = _resolve_cache_path(
                relate_with_cache=False, fresh_cache=False, cache_path=None
            )
            assert result == cache_file
    ...
    def test_neither_flag_with_custom_path(self, tmp_path):
        ...
        result = _resolve_cache_path(
            relate_with_cache=False, fresh_cache=False, cache_path=str(cache_file)
        )
        assert result == cache_file
