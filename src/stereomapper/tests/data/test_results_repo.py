...
class TestLoadAccession:
    ...
    def test_load_accession_existing_smiles(self, temp_cache_db):
        ...
        expected = {"CCO": "CHEMBL545", "CCN": "CHEMBL123"}
        assert result == expected
...
class TestPreloadClusterSru:
    ...
    def test_preload_cluster_sru_partial_matches(self, temp_results_db):
        ...
        assert result[cluster_id]["has_sru"]
...
class TestIntegration:
    ...
    def test_full_workflow_integration(self, temp_results_db):
        ...
        bulk_upsert_clusters(_conn, cluster_data)
        ...
        assert not sru_info[all_cluster_ids[0]]["has_sru"]  # none SRU
        assert sru_info[all_cluster_ids[1]]["has_sru"]  # def SRU
        assert sru_info[all_cluster_ids[1]]["rep"] == 3
        assert sru_info[all_cluster_ids[2]]["has_sru"]  # undef SRU
        assert sru_info[all_cluster_ids[2]]["is_undef"]
