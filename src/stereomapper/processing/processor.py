...
from stereomapper.domain.models import CacheEntry, ProcessingResult
from stereomapper.processing.sdf import CurieExtractor, SDFPropertyExtractor
...
class MoleculeProcessor:
    ...
    def process_molecule_metadata(self, path_str: str) -> dict[str, Any]:
        ...
        try:
            ...
            primary_curie = (
                self.curie_extractor.pick_primary_curie(sdf_props, curies) if curies else None
            )

            if primary_curie:
                result["accession_curie"] = primary_curie
            else:
                result["accession_curie"] = self.curie_extractor.fallback_accession(
                    file_path=path_str
                ) or assemblers.prefixed_identifier(
    ...
    def process_molecule_chemistry(
        self, path_str: str, canon_smiles: Optional[str] = None
        ...
        try:
            ...
            smiles = (
                canon_smiles
                if chosen_source == "canon"
                else smiles_orig if chosen_source == "orig" else None
            )
    ...
    def generate_molecule_key(
        self, inchikey_full: Optional[str], smiles: Optional[str], charge: int, file_hash: str
    ) -> str:
        ...
        if inchikey_full:
            ...
            return hashlib.blake2b(
                f"{self.std_version}|PARSE-ERROR|{file_hash}".encode(), digest_size=16
            ).hexdigest()
...
class CacheManager:
    ...
    def get_cache_metadata(
        self, molfile_paths: list[str]
    ) -> tuple[list[str], list[ProcessingResult], dict[str, dict[str, str]]]:
        """
    ...
    def _process_single_molecule(
        ...
        if not os.path.exists(path_str):
            ...
            molecule_key = hashlib.blake2b(
                f"{self.std_version}|MISSING|{file_hash}".encode(), digest_size=16
            ).hexdigest()
            smiles = None
            inchikey_first = None
            metadata = {
            ...
            (
                ...
                inchikey_first,
                inchikey_full,
            ) = self.molecule_processor.process_molecule_chemistry(path_str, canon_smiles)
