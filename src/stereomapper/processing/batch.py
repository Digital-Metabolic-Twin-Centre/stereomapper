...
logger, summary_logger = setup_logging(
    console=True, level="INFO", quiet_console=False, console_level="INFO"
)
...
class BatchProcessor:
    ...
    def process_batch(
        ...
        *,
        ...
        try:
            ...
            with section_timer(
                f"Processing {len(to_process)} new files in chunks of {self.chunk_size}", logger
            ):
            ...
            raise CacheError(
                f"Database error during batch processing: {str(e)}", operation="batch_processing"
            ).add_context("batch_size", len(molfile_list)) from e
        except Exception as e:
            raise BatchProcessingError(
                f"Unexpected error during batch processing: {str(e)}", batch_size=len(molfile_list)
            ) from e
    ...
    def _validate_batch_inputs(
        ...
        if not molfile_list:
            raise ValidationError(
                "Cannot process empty molecule list", field_name="molfile_list"
        ...
        if conn is None:
            raise ValidationError(
                "Database connection is required", field_name="conn"
    ...
    def _validate_and_resolve_paths(self, molfile_list: list[str]) -> list[str]:
        ...
        if invalid_files:
            logger.warning(
                f"Found {len(invalid_files)} invalid files, processing {len(files)} valid files"
            )
            # Log a few examples
    ...
    def _prepare_batch_metadata(
        self, files: list[str], conn: sqlite3.Connection, results: list[ProcessingResult]
        ...
        try:
            ...
            raise CacheError(
                f"Database error during cache lookup: {str(e)}", operation="cache_lookup"
