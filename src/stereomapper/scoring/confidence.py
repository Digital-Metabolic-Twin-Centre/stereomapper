    ...
    def score(
        ...
        *,
        ...
        if assigned_class in {"IDENTICAL", "IDENTICALS", "IDENTICAL_MOLECULES", "UNRESOLVED"}:
            ...
            score = 0.50 * stereo_consistency_mult + 0.25 * geom + 0.25 * tan2d
            contributors.update(
                {
            ...
            score = (0.50 * charge_mult + 0.25 * geom + 0.25 * tan2d) * stereo_consistency_mult
            contributors.update(
                {
            ...
            score = 0.50 * sm + 0.25 * geom + 0.25 * tan2d
            contributors.update(
                {
            ...
            score = (
                0.50 * stereo_consistency_mult + 0.25 * geom + 0.25 * tan2d - 0.10 * partial_penalty
            )
            contributors.update(
                {
                    "stereo_consistency_mult*0.50": 0.50 * stereo_consistency_mult,
            ...
            score = (
                0.50 * stereo_consistency_mult + 0.25 * geom + 0.25 * tan2d
            )  # using diff term unfairly penalises diastereomers with many stereo centres matching stereochemistry
            ...
            contributors.update(
                {
                    "stereo_opposite*0.50": 0.50
                    * stereo_consistency_mult,  # Match what you're actually using
                    "geom*0.25": 0.25 * geom,
                    "tanimoto2d*0.25": 0.25 * tan2d,
                }
            )
            expect = {
                "ik_first_eq": 1,
                "charge_match": 1,
            }  # would expect one of strucs to have 2nd layer 'UHHF..

        else:  # fallback
            diff_term = so if so > 0 else (1.0 - sm)
            score = 0.50 * diff_term + 0.25 * geom + 0.25 * tan2d
            contributors.update(
                {
        ...
        score = int(round(100 * clamp01(score)))
        ...
        expectations = {
            ...
            "observed": {
                "ik_first_eq": ik1,
                "ik_stereo_layer_eq": iks,
                "ik_protonation_layer_eq": ikp,
                "charge_match": chg,
            },
        }
