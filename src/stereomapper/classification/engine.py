...
from stereomapper.domain.chemistry import ChemistryOperations, ChemistryUtils, StereoAnalyser
...
class RelationshipAnalyser:
    ...
    def calc_relationship(
        ...
        if (has_sru1 or has_sru2) and not (
            (has_sru1 != has_sru2)
            or (has_sru1 and (is_undef_sru1 != is_undef_sru2))
            or (
                has_sru1
                and (not is_undef_sru1)
                and (sru_repeat_count1 is not None)
                and (sru_repeat_count2 is not None)
                and (sru_repeat_count1 != sru_repeat_count2)
            )
        ...
        if rmsd_err:
            return SimilarityResult(
                classification="RMSD_ERROR", rmsd=None, details={"error": rmsd_err}
            )
        ...
        num_unspecified = stereo_elements.get(
            "unspecified", 0
        )  # not used, but could be useful in the future
        num_stereogenic_elements = stereo_elements.get(
            "total_stereo"
        )  # if stereo_elements['details'] else 1 # this is problematic I think - stereocentres can infact be zero
        ...
        if num_stereogenic_elements == 0:
            ...
            if rmsd is not None:
                if charge1 != charge2:
                    ...
                    return _to_similarity_result(
                        StereoClassification.identical_missing_charge(
                            stereo_score=conf.score,
                            confidence=conf.as_dict(),
                            rmsd=rmsd,
                            num_stereogenic_elements=num_stereogenic_elements,
                    ...
                    return _to_similarity_result(
                        StereoClassification.unresolved(
                            ...
                            num_missing=num_missing,
                            details={
                                "reason": "Possible pipeline error - should be no identical relationships in this phase"
                            },
                        )
                    )
            else:
                return _to_similarity_result(
                    StereoClassification.indistinguishable_structures(rmsd=rmsd)
                )
        ...
        is_identical = (
            (num_flips == 0)
            and (num_missing == 0)
            and ((num_matches + num_unspecified) == num_stereogenic_elements)  # should work
        )
            ...
            if StereochemicalClassifier.is_enantiomer(stereo_elements):
                ...
                if charge1 != charge2:
                    ...
                    return SimilarityResult(
                        ...
                        details={
                            **(res.details or {}),
                            "reason": "Parent-child stereochemical relationships must share protonation/charge; no valid class",
                        },
                    )
                else:
                    conf = builder.build_features_for_confidence(
