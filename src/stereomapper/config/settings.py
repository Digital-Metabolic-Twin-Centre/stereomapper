    ...
    def validate(self) -> None:
        ...
        if self.chunk_size <= 0:
            raise ConfigurationError(
                "chunk_size must be positive", config_field="processing.chunk_size"
            )

        if self.max_workers <= 0:
            raise ConfigurationError(
                "max_workers must be positive", config_field="processing.max_workers"
            )
...
class ChemistrySettings:
    ...
    def validate(self) -> None:
        ...
        if self.rmsd_threshold < 0:
            raise ConfigurationError(
                "RMSD threshold must be non-negative", config_field="chemistry.rmsd_threshold"
            )
...
class Settings:
    ...
    def _validate_input_sources(self) -> None:
        ...
        if has_files and has_directory:
            raise ConfigurationError(
                "Cannot specify both input_files and input_directory", config_field="input_sources"
