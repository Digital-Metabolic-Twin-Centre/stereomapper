...
class PipelineConfig:
    ...
    @classmethod
    def from_settings(cls, settings: Settings, inputs: Optional[List[str]] = None, output_path: Optional[str] = None) -> 'PipelineConfig':
        """Create PipelineConfig from Settings for backward compatibility."""
        return cls(
            input=inputs or ([str(f) for f in settings.input_files] if settings.input_files else None),
            input_dir=str(settings.input_directory) if settings.input_directory and not inputs else None,
            recursive=getattr(settings, 'recursive', False),
            sqlite_output_path=output_path or (str(settings.database.output_path) if settings.database.output_path else None),
