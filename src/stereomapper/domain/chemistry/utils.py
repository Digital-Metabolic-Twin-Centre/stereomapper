...
class ChemistryUtils:
    ...
    def create_temp_molfile(mol: Chem.Mol) -> Optional[str]:
        ...
        try:
            ...
            temp_file = tempfile.NamedTemporaryFile(mode="w", suffix=".mol", delete=False)
    ...
    def validate_molecule_data(
        smiles: Optional[str] = None, inchikey: Optional[str] = None, formula: Optional[str] = None
    ) -> dict[str, bool]:
        ...
        if validation["has_inchikey"]:
            ...
            validation["inchikey_valid"] = (
                len(inchikey) == 27
                and inchikey[14] == "-"
                and all(c.isalnum() or c == "-" for c in inchikey)
            )
    ...
    @staticmethod
    def normalise_rmsd(
        rmsd_result: Union[float, dict[str, Any], None],
